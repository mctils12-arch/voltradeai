...
## 2026-07-10 — [REPAIR] gv-div4-ks orphaned pod (2h13m unwatched billing) found+closed; the actual train_rc=1 root cause (holdout region never fetched) fixed+tested; runpod_reap.py stopgap shipped (v1.0.262)

TERRITORY: T-DATACORE (scripts/gridvision_train/**, scripts/runpod_*.py, research/). Continuation of the prior 2026-07-10 [PIPELINE] session's in-flight gv-div4-ks launch (new session — the prior session's own container/watchdog ended when it did, per the OPTION A CAVEAT).

SESSION-START CHECKS: read CLAUDE.md in full. `git fetch origin main` — local branch was already at main's tip (8975ff3, PR #413 merged), 0 commits ahead/behind; designated branch `claude/dazzling-planck-g1d293` didn't exist remotely yet (created this session). `/api/health`: status ok, bot active, drawdownPct 0.0, liveness.dark false, alpaca ACTIVE — no LIVENESS ALARM. KNOWN BROKEN (open_questions.md): re-verified nothing newly critical-unfixed — items #3/#4/#16/#17 are either already-addressed-and-noted-stale or low-priority/self-healing per their own text; #12(b)/(c) and #15 are gated on time passing, not actionable. Last-10 experiments.md entries: 1 REPAIR / 1 PRODUCT / 6 PRODUCT / 1 RESEARCH / 1 PIPELINE — no thrash.

PRIMARY ACTION SELECTION: the prior session's entry (line above) explicitly named the follow-up: check `datacore/runpod/ledger.jsonl` + the `gridvision-pod-result-gv-div4-ks` branch for gv-div4-ks's outcome before doing anything else GRID-VISION-related. Did that first — and it surfaced a live, active-right-now financial-safety issue (GOAL priority 1) that outranked anything else queued, so this became the session's [REPAIR] primary action per the Repair Mandate.

WHAT WAS FOUND:

1. **gv-div4-ks's training crashed in ~9 minutes**, not the 5-hour cap: the pushed log (`gridvision-pod-result-gv-div4-ks` branch, `train_rc=1`) showed `FileNotFoundError: No images found in .../images/val`. ROOT CAUSE (not the GSD bug the prior session fixed — that fix was never exercised, training never started): the div4 launch command's `--regions` list (AZ + 5 NZ + 3 Duke) never included `--holdout-region USA_KS_Colwich_Maize` — `fetch_etdii_us` only downloads what's in `--regions`, so KS was never fetched, guaranteeing `build_yolo_dataset`'s `val_ids = [s for s in stems if e2y.region_of(s) == holdout_region]` was empty. A pure invocation mistake with no validation to catch it before spending pod time. FIX: extracted the split logic into a standalone pure `compute_holdout_split()` that raises a clear `ValueError` if it would produce 0 val images (protects every caller, not just the CLI), PLUS a `validate_holdout_in_regions()` pre-flight check called in `run_full()` BEFORE any network fetch — a future misconfiguration like this fails in <1s with an actionable message instead of ~9 minutes + a pip-install + model-download into a cryptic ultralytics traceback. 5 new tests in test_gridvision_train.py (35/35 pass, was 30).

2. **gv-div4-ks's POD (`h7sxegyzvjqwqm`) was still `RUNNING` and billing 2h13m after the crash**, discovered via `GET /pods/h7sxegyzvjqwqm` (status RUNNING, `desiredStatus RUNNING`, `lastStartedAt` = pod creation time, no live process watching it). ROOT CAUSE: `research/runpod_ledger.md`'s OPTION A CAVEAT (human-accepted 2026-07-08) — the cost-cap watchdog lives in the launching session's process. This repo's sessions run in an EPHEMERAL container reclaimed when the session ends; the prior session ended (as sessions do) before its watchdog loop's terminate step ran, orphaning the pod with literally nothing watching it. VERIFIED the existing "in-pod `timeout` wrapper" backstop does NOT help here despite the launcher's own docstring implying it does: `timeout` bounds the wrapped PROCESS (which did exit, at the crash), not the POD's billing meter — a RunPod pod bills until `DELETE`d regardless of what its start command is doing. This is a real gap in the cost-cap gate's "hard ceiling" guarantee, not previously demonstrated live.

IMMEDIATE MITIGATION (before any other work, given active $ bleed): `DELETE /pods/h7sxegyzvjqwqm` — confirmed gone (subsequent `GET` returned 404). Closed the ledger row: actual cost **$1.5389** (2h13m41s @ $0.69/hr) vs $3.45 reserved — no runaway, but purely because the training crash happened to be cheap+early, not because anything would have stopped it at 5h if the crash hadn't happened. Balance: $45.25 of $50 remaining, 0 open jobs (`python3 scripts/runpod_budget.py status` reconfirmed clean after the close).

STRUCTURAL FIX (stopgap, not the full fix): built `scripts/runpod_reap.py` — pure `find_open_jobs`/`match_orphans`/`estimate_cost_for_gone_pod` helpers (ledger open-job vs. `GET /pods` matching by `pod.name == job_id`, since `build_create_body` always names the pod after the job) + a `run_reap(apply=False)` orchestrator mirroring `run_launch`'s awaiting-key gate. Dry-run by default; `--apply` terminates any still-live orphan and closes the ledger (capped at the job's own reserved cost if the pod's already gone, so a reap can never invent a cost above what was already authorized). 10 new tests (network fully monkeypatched). Documented in `research/runpod_ledger.md` as the standing session-start check for any GRID VISION RunPod work going forward. Did NOT build the real fix (Option B: a server-side watchdog in the always-on Railway process, immune to any one session ending) — that needs `RUNPOD_API_KEY` moved into the deployed app (a broader exposure surface, a human call) and is its own PR-sized effort; filed as a proposal in `research/wishlist.md` given this is now the 5th recurring GPU launch (div1-div5), past the "if it becomes recurring" trigger `runpod_ledger.md` already named for Option B.

NOT DONE THIS SESSION (deliberate): did not relaunch a corrected gv-div5-ks. The exact corrected launch command (regions list now includes `USA_KS_Colwich_Maize`, `--max-hours` lowered 5→3 given div2's comparable-scale run finished in 29 min) is filed in `research/grid_vision_phaseb.md`'s 2026-07-10 follow-up section — relaunching needs a session that stays attached to the watchdog for the run's duration (or immediately reaps afterward), and this session's remaining scope went to closing the two bugs just found instead of opening a new multi-hour in-flight job right after diagnosing exactly this failure mode.

GATES: `python3 -m pytest -q` — 616 (prior session's own stated baseline) → 631 passed, 1 skipped (pre-existing legacy skip, unchanged), zero regressions; +15 new tests total (5 in test_gridvision_train.py, 10 in test_runpod_reap.py), confirmed at two checkpoints this session (621 after the train.py fix, 631 after runpod_reap.py). `python3 scripts/gridvision_train/train.py --dry-run` — all 9 checks pass, including the pre-existing `duke_native_gsd_fix` check (unaffected by this session's split-logic refactor). `python3 scripts/runpod_reap.py` against the live ledger — correctly reports "no open ledger jobs" post-close.

Backtest: N/A (offline detector-training infra + cost-cap tooling; no trading/measurement/parameter path touched).

Version 1.0.261 → 1.0.262 (read-and-increment at commit time).
