...
RESULT: the corrected retry (10 regions fetched — AZ + 5NZ + 3 Duke US + KS itself for val — GSD-corrected per the earlier fix, held out KS, yolov8s, strong aug, 80 epochs) trained cleanly this time (`train_rc=0`) and completed in **1146s (19 min)**, terminated instantly by the new `--result-branch` watchdog the moment `pod_run.py` pushed (`actual_cost $0.22`, not the $3.45 worst-case — the early-termination fix from the prior entry paid for itself many times over on its very first real use). Held-out KS: **AP50 0.197, AP50-95 0.054, precision 0.330, recall 0.250** (995 train images / 4,754 boxes, 461 val tiles from 48 KS images / 1,415 boxes — `native_gsd_by_region` in the log confirms the GSD fix applied correctly on real data: Duke regions at 0.15, everyone else at 0.30).

HONEST COMPARISON (REASONING STANDARD #4/#10 — stating the prior before crediting anything): the pre-registered hypothesis (research/grid_vision_gate1.md's diversity trend line, 2026-07-09) was "not saturated at 6 regions... the trend line says more regions get there" — i.e., adding the 3 real Duke US regions on top of the existing AZ+5NZ 6-region set (which scored 0.200 held-out on this same KS fold) was expected to climb further toward the 0.30 bar. It did NOT: 0.197 vs 0.200 is a **-0.003 delta, inside the leaderboard's own 0.02 noise threshold** (`gridvision_leaderboard.py add` classified it `"within_noise"`, not a regression, not an improvement — logged to `datacore/gridvision/leaderboard.json` as `div5-ks`). Region-count diversity has PLATEAUED at this scale, contradicting the earlier trend read. Two live hypotheses for WHY, neither yet tested (ladder-honest — not claimed until tested): (a) Duke's imagery has real radiometric/sensor characteristics beyond GSD (the ETDII/NZ set is all one source/processing pipeline; Duke's E&TD deposit is a different survey) that the GSD-only correction doesn't close — the model may be learning source-specific texture, not region-invariant tower features; (b) 9-10 training regions may simply not be enough raw EXAMPLE COUNT yet regardless of diversity (still <150 total training images) — the earlier "compounds, not saturated" read from 1→6 regions may have been riding example-count growth as much as diversity per se, and Duke's 3 regions added proportionally fewer new examples relative to the dataset's existing size than NZ's 5 did earlier.

WHAT THIS SESSION ACTUALLY SHIPPED (full arc, 3 PRs): (1) #413 — found+fixed a real Duke native-GSD bug (was silently training Duke at 2x too coarse a GSD; never yet corrupted a completed run, but would have on this one) + built `pod_run.py` (on-pod crash-survives-and-pushes-a-log wrapper, closing a real "no durable pod log" gap 2 prior sessions each solved ad hoc and left uncommitted); (2) #416 — found+fixed the watchdog idle-billing gap (a fast finish/crash billed the full cap because the base image never reports the pod exited) + a real test-isolation bug in `runpod_budget.py` (a monkeypatch that silently didn't redirect, caught only because it polluted the real ledger with 4 fake rows — cleaned + fixed + regression-tested); (3) this entry — the actual experiment result, honestly reported as a plateau, not spun as either a win or a wasted session. Every one of the three infra fixes was DISCOVERED BY, not planned before, actually running the real pipeline end-to-end for the first time — exactly the value of moving from "designed but never run" to "run for real," per EDGE DOCTRINE #3 (compile knowledge into code once discovered, never re-risk it).

COST (full session): gv-div4-ks $3.451 (8min failure, idle-billed to cap — the bug this session's own PR then fixed) + gv-div5-ks $0.220 (19min, real success, early-terminated) = **$3.67** this session. Ledger balance **$43.12 / $50.00** (from $46.79 at session start). Two smaller earlier test-pollution rows ($0, no real RunPod calls, cleaned) not counted.

NEXT (logged, not started — session capacity call, not a blocker): test hypothesis (b) first (cheaper to test — just keep adding real-example COUNT within existing regions before assuming diversity itself is capped) before concluding diversity is genuinely saturated; if (b) doesn't move the needle, hypothesis (a) (domain-adapt or radiometric-normalize Duke's imagery toward the ETDII/NZ appearance before training) is the next lever. The REVISED STRATEGY's other queued options (research/experiments.md 2026-07-09 entry: NAIP self-bootstrap, Nevada rollout as a measured-low-confidence bootstrap) remain queued and un-superseded by this finding.

Backtest: N/A (offline detector training / research finding; no trading/measurement/parameter path touched).

## 2026-07-10 — [PIPELINE] GRID VISION: gv-div4-ks FAILED fast (own launch-arg mistake, not a code bug) — dataset built with ZERO val images; found + fixed two more infra gaps (watchdog idle-billing, ledger test-isolation); gv-div5-ks (corrected) launched (v1.0.262, PR #416)

TERRITORY: T-DATACORE. Continues the same session as the entry directly below (2026-07-10 v1.0.261) — same territory declaration applies.

RESULT OF gv-div4-ks (launched in the prior entry): FAILED, `train_rc=1`, in ~8 minutes — not a code defect, a mistake in the launch command I wrote. Diagnosed from the pushed log (`git show origin/gridvision-pod-result-gv-div4-ks:gv_train.log`; `pod_run.py` worked exactly as designed — captured the crash and pushed the log instead of losing it): the dataset build report showed `"images": {"train": 995, "val": 0}` — I passed `--regions` with the 9 DIVERSITY regions (AZ + 5NZ + 3 Duke US) but forgot to also include `USA_KS_Colwich_Maize`, the `--holdout-region` itself. `fetch_etdii_us` only downloads what's in `--regions`, so no KS zip was ever fetched, `build_yolo_dataset`'s `val_ids = [s for s in stems if region_of(s) == holdout_region]` found nothing, and Ultralytics crashed immediately at trainer setup (`FileNotFoundError: No images found in .../images/val`). The held-out region must ALSO be in `--regions` — `--holdout-region` only selects which of the fetched regions becomes val, it does not fetch anything itself. (Confirms the 995-train-image dataset build itself was otherwise healthy: 4,754 boxes, 0 unmapped-region stems — the new per-region GSD fix from the prior entry worked correctly on real data, `native_gsd_by_region` in the log shows AZ/NZ at 0.3 and all 3 Duke regions at 0.15 as intended.)

SECOND FINDING (real waste, not my mistake — infra gap): despite failing in ~8 minutes, the pod kept billing for the FULL 5-hour cap ($3.45 for ~8 min of real work), because the `runpod/pytorch` base image keeps jupyter/ssh alive regardless of the launched command — RunPod's `GET /pods/{id}` status field never reported the pod as exited even though the training process (and the whole `--cmd` chain) had long since finished and returned. (The watchdog log shows status=RUNNING continuously from t=21s to t=17692s, then starts 404ing — the pod record itself eventually vanished from RunPod's API, cause unconfirmed, possibly a SECURE-pod cleanup window — before finally hitting the wall-clock cap at t=18003s.) This is the EXACT same waste div2 hit (research/experiments.md 2026-07-09) and solved ad hoc with an uncommitted git-branch-SHA watch each time. Fixed properly this session (own PR, #416): `runpod_launch.py --result-branch <branch>` makes the watchdog check `git ls-remote origin refs/heads/<branch>` every poll (via this session's own git proxy — no new token) and terminate the INSTANT the branch appears, wired as `watchdog_should_terminate`'s new `result_ready` param (checked last, after natural pod-exit detection, so a naturally-EXITED pod still reports "exited" not "result-pushed").

THIRD FINDING (test-isolation bug, caught while adding coverage for the second fix): `runpod_budget.py`'s `load_ledger()`/`_append_row()` defaulted `path=LEDGER` — a value bound ONCE at module-DEF time, the classic Python mutable-default-argument trap. A new test for `run_launch()`'s early-termination wiring did `monkeypatch.setattr(rb, "LEDGER", tmp_path)` and called through to `_append_row()` with no explicit path, exactly as `run_launch()`'s own internal reservation/close calls do — and because the default was bound at def time, the monkeypatch did NOT redirect it: the test silently wrote 4 fake rows (`job_id: "j"`) into the REAL `datacore/runpod/ledger.jsonl`. No real money was at risk (`actual_cost: 0`, no real RunPod call made — `create_pod`/`terminate_pod` were themselves monkeypatched), but the production data file was polluted and had to be manually cleaned (`git diff` confirmed only the legitimate gv-div4-ks close row remained afterward). Fixed both functions to resolve `LEDGER` at call time (`path=None` default, resolved inside the function body) instead of def time; added a regression test that specifically proves the monkeypatch now redirects both functions when called with no explicit path.

GATES: full `python3 -m pytest -q` — 624 passed, 1 pre-existing skip. Verified twice more that a full suite run leaves `datacore/runpod/ledger.jsonl`'s git diff containing ONLY the legitimate gv-div4-ks close row (no re-pollution).

LAUNCH: **gv-div5-ks** — identical config to div4 but with `USA_KS_Colwich_Maize` correctly included in `--regions` (10 regions total: 9 diversity + the held-out one itself) and `--result-branch gridvision-pod-result-gv-div5-ks` wired for early termination. SECURE non-interruptible, max-hours 5 (worst-case $3.45 unchanged — the early-termination fix lowers the EXPECTED cost, not the safety ceiling), ledger reserved, gate authorized ($43.34 balance before this reservation). Pod `6fs2vfbbybgnzs` created; watchdog running in a harness-tracked background task. **RESULT NOT YET KNOWN AT THIS ENTRY'S WRITE TIME** — appended once known, per the same protocol as the entry below. Check `datacore/runpod/ledger.jsonl` (job `gv-div5-ks`) and branch `gridvision-pod-result-gv-div5-ks` before launching anything further if reading this mid-run.

Backtest: N/A (offline detector training infra; no trading/measurement/parameter path touched).

## 2026-07-10 — [PIPELINE] GRID VISION: fixed Duke native-GSD bug + on-pod result-push wrapper; gv-div4-ks (Duke-inclusive diversity retrain) LAUNCHED, in flight at session end (v1.0.261)

TERRITORY: T-DATACORE (scripts/gridvision_train/**, scripts/runpod_launch.py, research). [PRODUCT] session per the standing routine's charter (datacore/ pipelines + /data surface).

SESSION-START CHECKS: read CLAUDE.md in full. `/api/health`: status ok, bot active, drawdownPct 0.0, liveness.dark false, alpaca ACTIVE, scanner 0 consecutiveFailures — no LIVENESS ALARM, no repair-duty preemption needed. Last-10 experiments.md entries: 1 REPAIR / 8 PRODUCT / 1 RESEARCH — healthy, no thrash. Audit register: staleness due 2026-08-04, constitutional due ~2026-08-03 — neither overdue. Verified R19's fix (previous session) live via `/api/diag/audit?type=STREAM`: "Real-time feed live — 43 tickers" now appears repeatedly (was silent for 22h+ before); WS-EXIT still empty (no tracked position has hit a stop/target since the deploy yet — consistent with KNOWN BROKEN #12(b)'s gate, not a new break). `/api/diag/ml`: feedback_seeded_count 1326 (healthy), feedback_live_count still 0 (expected, gated on a live WS exit per #12(b)).

PRIMARY ACTION SELECTION: KNOWN BROKEN had nothing newly actionable and unfixed-critical (R19 already shipped and live-verified above). Surveyed queued PRODUCT/PIPELINE work (console_charter.md fully shipped as of #410; orbital_program.md's unblocked next items are O5/O7, both lower-value showcase work per GOAL's self-proposed-work ordering) and found the GRID VISION detector-training arc (research/grid_vision_phaseb.md, research/experiments.md 2026-07-09 div2/div3 entries) both **RunPod-unblocked** (RUNPOD_API_KEY AND GRIDVISION_GH_PAT both present in this session's env — first time both have been available together) and **explicitly queued as the next step** (div3's own resume note: "crossing 0.30 with real US regions needs offline Duke pre-downsampling next"). This is category (2) validated-signal-product work, GOAL's highest self-proposed priority band — chosen over showcase items.

READ BEFORE WRITE finding (BUG, not yet corrupting anything): `train.py`'s `build_yolo_dataset` computed ONE global downsample factor from `ETDII_NATIVE_GSD` (0.30 m) and applied it to every training image regardless of source. But `gridvision_etdii.py`'s own DUKE_ADD comment documents Duke US regions (Hartford CT / Clyde NC / Wilmington NC) as natively **0.15 m**, not 0.30 m — they need factor 4.0 (0.15→0.60), not 2.0. Under the old code, Duke images would have landed at an effective 0.30 m instead of the intended 0.60 m target — a real cross-region SCALE mismatch (Duke towers ~2x the true pixel footprint of every AZ/KS/NZ tower in the same dataset), silently undermining the entire diversity experiment's premise (region-invariant appearance) with a GSD confound baked into the data itself. Root-cause check: had this already corrupted a completed run? No — both prior Duke attempts (gv-div2-ks BadZipFile, gv-div3-ks 2h-cap timeout) failed BEFORE reaching the training step, so this bug had zero completed victims yet, but would have corrupted the very next one silently (no crash, no error — just a wrong number quietly baked into every Duke box).

FIX (own logical unit, tests pass before any GPU dollar spent): `REGION_NATIVE_GSD` table (ETDII US+NZ → 0.30 m, Duke US → 0.15 m) + `native_gsd_for_stem`/`region_of_any` (covers ALL regions incl. NZ, unlike `e2y.region_of` which is deliberately US-only-scoped for the held-out-region test) + per-image factor computation inside `build_yolo_dataset`'s main loop (was: one factor computed once outside the loop). Return dict now reports `native_gsd_by_region` (honest per-run record of what was actually applied) instead of a single possibly-wrong `downsample_factor` scalar. `dry_run()` check #9 locks in: AZ/KS/NZ → 0.30 m, Duke → 0.15 m → factor 4.0, unknown region → ETDII default (never crashes). 4 new pytest cases in test_gridvision_train.py.

SECOND FIX (closing an OPS GOTCHA, not a correctness bug): neither `train.py` nor `runpod_launch.py` captures pod stdout/logs anywhere durable — a crashed run left nothing to diagnose beyond the ledger's bare cost number. Prior sessions (div2/div3) solved this ad hoc with an UN-COMMITTED git-push wrapper each time (the "gridvision-pod-result" branches mentioned in those entries never existed in any PR diff — confirmed by `git show` on both commits, which touch only ledger/leaderboard JSON). Per EDGE DOCTRINE #3 (compile knowledge into code, never re-risk the same solved problem on the next GPU dollar), built `scripts/gridvision_train/pod_run.py`: runs train.py as a SEPARATE SUBPROCESS (a training crash can't take the wrapper down with it) capturing all stdout/stderr to a log file unconditionally, then pushes the log (always) + gv_best.pt (if training got that far) to a **per-job-id** git branch (`gridvision-pod-result-<job_id>` — prior ad hoc runs reused one branch name across jobs, leaving untraceable leftover-branch clutter, per the LEFTOVERS note in the 2026-07-09 div0 entry; per-job naming fixes that too). The git command sequence (pure `build_push_plan`) is unit-tested AND exercised end-to-end against a real throwaway local bare repo (no network, no RunPod) before ever touching a live pod — 11 new tests, including one that proves the orphan-branch mechanics push exactly the 2 result files and nothing else. Also added `--cloud-type`/`--non-interruptible` CLI flags to `runpod_launch.py` (previously only reachable by calling `run_launch()` directly, undocumented) — 3 new tests.

GATES: full `python3 -m pytest -q` (after `pip install -r requirements.txt -r requirements-dev.txt`, absent in this fresh sandbox) — 616 passed, 1 pre-existing skip (legacy backtest_v1028_full), zero regressions. `train.py --dry-run` passes all 9 checks including the new GSD one.

LAUNCH: pushed this branch (claude/quirky-hopper-t6tnva), then launched **gv-div4-ks** — the corrected version of div3's attempt: yolov8s, strong aug, 9 training regions (AZ + 5NZ + 3 Duke US), held out KS, 80 epochs, via `pod_run.py` (log+weights survive regardless of outcome). Learned from gv-div1-1's COMMUNITY-spot PREEMPTION (2026-07-09 entry): this run uses **SECURE non-interruptible** ($0.69/hr) since `pod_run.py` doesn't checkpoint mid-run — a preemption would still lose everything. `max-hours 5` (vs div3's 2h that ran out mid-download) — worst case **$3.45**, ledger reserved before creation, $43.34 would remain; gate authorized. Pod `h7sxegyzvjqwqm` created; watchdog running in a harness-tracked background task (polls every 45s for the launcher's own "terminated" log line, 5h50m safety ceiling). **RESULT NOT YET KNOWN AT THIS ENTRY'S WRITE TIME** — this session is actively monitoring and will append the outcome (AP50 vs the pre-stated ≥0.30 held-out bar, cost, any infra finding) the moment the watchdog reports. If a future session reads this before that update lands, check `datacore/runpod/ledger.jsonl` for `gv-div4-ks` and the git branch `gridvision-pod-result-gv-div4-ks` for the log/weights directly — do not re-launch without checking those first (RECURRENCE ESCALATES — a fresh gv-div5 attempt on top of an unexamined gv-div4 result would waste a GPU dollar re-deriving what's already sitting in the ledger).
...
## 2026-07-10 — [REPAIR] SPY floor rebalancer submitted live market orders around the clock, no market-hours check anywhere (v1.0.263)

TERRITORY: T-BOT (`bot_engine.py`), plus a new root-level test file and the
two research/* docs updates SESSION BUDGET requires (shared-file, minimal,
last commit).

SESSION-START CHECKS (MEMORY PROTOCOL): read CLAUDE.md in full, then
experiments.md's last 10 tagged sessions for the loop-health ratio
([REPAIR][PIPELINE][PIPELINE][PIPELINE][RESEARCH][REPAIR][PRODUCT]
[RESEARCH][PRODUCT][PRODUCT] — 2/10 REPAIR, nowhere near the 7+ thrash
trigger, so no meta-problem to address first), open_questions.md KNOWN
BROKEN, wishlist.md. `/api/health` clean (bot active, drawdownPct 0.0,
liveness not dark, scanner not degraded) — no liveness alarm, no
higher-priority item queued above KNOWN BROKEN.

PRIMARY ACTION CHOICE: per the REPAIR MANDATE ("known breaks outrank new
research — session one onward: consult open_questions.md KNOWN BROKEN
first"), queried the token-gated `/api/diag/*` probes exactly as KNOWN
BROKEN #15's own NEXT step instructed (worked example already in this
file) to check what a prior session left pending. Two things came back:
(a) KNOWN BROKEN #15 (WS real-time exit feed) is CONFIRMED FIXED live —
`/api/diag/audit?type=STREAM` shows "Real-time feed live — 43 tickers"
every ~10 min, no action needed, noted here for the record so a future
session doesn't re-investigate it. (b) `/api/diag/orders` showed KNOWN
BROKEN #16 (extended-hours market orders "rejected and blindly retried")
is not just a historical finding — it is CURRENTLY FIRING: 50 canceled +
31 pending "new" SMH buy-market orders submitted ~5 minutes apart, spanning
2026-07-07 through the moment of this session (2026-07-10, pre-market ET),
continuing to fire live during this investigation. A confirmed, live,
currently-active, previously-diagnosed-but-unrepaired bug is exactly what
the REPAIR MANDATE prioritizes above starting new research — chosen as the
session's one action.

DIAGNOSIS (READ BEFORE WRITE): KNOWN BROKEN #16's own NEXT step asked
"confirm whether `getOrderParams`'s `extended_hours` branch (KNOWN BROKEN
#8) is actually reached for this order type, or whether this path bypasses
it entirely." Traced `server/bot.ts`'s order-submission call sites first
(`executeTrades`, the Tier-3 `BUY`/`BUY_PUT` tier-action dispatcher, scale-
outs, position kills) — every one of them correctly calls `getOrderParams`
and is either gated by `isMarketOpen` or produces a proper extended-hours
limit order. None of them touch SMH, and SMH isn't a scanner candidate
ticker — SMH only appears as a `FLOOR_BASKET` member (`system_config.py`
line 128, weight 0.15) and in `bot.ts`'s `floorTickers` set (used only for
the 14-day stale-position TIME-EXIT skip, not order submission). That
pointed at the Python side: `bot_engine.py`'s `_manage_spy_floor()`
(passive floor-basket rebalancer, called unconditionally every Tier-2 scan
cycle — no `isMarketOpen` check anywhere in the function) submits orders
via direct Python `requests.post` calls to Alpaca, completely bypassing
`server/bot.ts` and `orderParams.ts` — a code path KNOWN BROKEN #8's fix
(Node-side only) could never have reached. Its `FLOOR_BASKET_ENABLED`
branch (added 2026-04-22) hardcodes `"type": "market"` with zero
market-hours awareness at all — the exact match for the live evidence
(SMH is basket-weighted 0.15, confirming this branch is the one actually
live in production). The legacy single-ticker branch (used when
`FLOOR_BASKET_ENABLED=False`) already uses limit orders per an earlier
2026-04-10 fix note but still carries no `extended_hours` flag and no
market-hours gate — same defect class, lower observed blast radius (a
day-limit order queues instead of getting rejected) but capable of
resubmitting duplicate pending orders every cycle if ever activated.

DOWNSTREAM CHAIN (REASONING STANDARD #1): a market order submitted outside
9:30am-4:00pm ET is invalid without `extended_hours: true` -> Alpaca
rejects/cancels it -> the position never changes -> `_manage_spy_floor`'s
own drift check (dollars-off-target vs. current holdings) sees the exact
same gap next cycle -> resubmits -> unbounded loop, ~5 minutes per Tier-2
cycle, burning API calls and Alpaca's day-order-count limits for zero
executional benefit, for as long as the market stays closed. No position-
sizing, scoring, or signal logic was touched by the bug or the fix — this
is purely "when is an order allowed to be submitted," the same category as
KNOWN BROKEN #8.

FIX: added `_is_regular_hours()` to `bot_engine.py` — copied directly from
the already-established, already-tested convention already living in
`options_scanner.py` (same DST-aware `datetime.now(ZoneInfo("America/
New_York"))` construction, same 9:30-16:00 ET bounds) rather than
inventing a new pattern. Gated every direct order-submission call inside
`_manage_spy_floor`: basket buy, basket sell, legacy single-ticker
buy/sell, and the `target_pct<=0` regime-triggered full exit. When closed,
no order is submitted and an honest `*_deferred` action is recorded
(`buy_deferred`/`sell_deferred` for the basket path, mirroring existing
`buy_failed`/`sell_failed` action-naming conventions already in the same
function; `floor_rebalance_deferred` for the legacy path;
`floor_exit_deferred` for the zero-target exit) instead of either
submitting a doomed order or silently dropping the rebalance. The basket
and legacy paths recompute drift from LIVE positions every single cycle
(not a one-shot flag), so a deferred cycle safely retries itself the
moment the market reopens — no extra state needed. The regime-exit branch
needed one extra correctness fix beyond the simple defer: it only fires
`if regime_changed`, and the function unconditionally persists
`_floor_state["last_regime"] = regime` right after that block — so a
deferred exit that still let the state-save proceed would flip
`regime_changed` to False on the next call and the exit would be silently
forgotten forever, not just delayed. Fixed by skipping the state-persist
specifically when deferred (new `_exit_deferred` flag), leaving
`last_regime` stale so `regime_changed` stays True until the exit actually
executes — verified directly by a test that seeds the on-disk state file,
calls the function with market closed, and asserts the file is byte-
identical afterward.

RATCHET COMPLICATION AND RESOLUTION: the first version of `_is_regular_hours`
used a bare `except Exception: return False`, mirroring options_scanner's
literal source — `test_silent_except_ratchet.py` (the loop-health-thrash
follow-through from 2026-07-06) correctly failed on it (bot_engine.py pin
78 -> 79, a genuinely NEW silent broad-except handler). Per that ratchet's
own stated remedies ("narrow the except to the specific exception type the
code genuinely intends to tolerate"), narrowed to `except
ZoneInfoNotFoundError` (tzdata missing — the one real failure mode of this
specific call) instead of bumping the pin, which CLAUDE.md's FROZEN PATHS
section forbids regardless ("Raising any pin is weakening a test"). This
is a small but real finding for whoever next touches options_scanner.py:
its own identical-shaped `_is_regular_hours` (line 173) still carries the
broad, unnarrowed `except Exception: return False` and is presumably
already inside options_scanner.py's own pin of 14 — not touched this
session (different file, different territory boundary, not the live bug),
but the same narrowing would apply there too if that file is ever edited
for an unrelated reason.

RATCHET (per HEALTH OF THE LOOP rule 3 — no repair without a regression
test): new `test_spy_floor_market_hours.py`. This function had ZERO test
coverage before this PR, for either the pre-fix or post-fix behavior —
notable given it submits live orders on a schedule. 5 tests, all mocking
every Alpaca call (no live network dependency): basket buy defers with no
`requests.post` call when closed; basket sell defers likewise; basket buy
STILL executes with the correct `type: market` payload when the market is
open (a regression guard proving the fix doesn't quietly disable the
floor strategy entirely); legacy single-ticker path defers when closed;
and the regime-exit-preserves-retry case (seeds `last_regime=BEAR` on
disk, calls with `SPY_FLOOR_BULL=0` and market closed, asserts no POST and
that the on-disk `last_regime` is still `BEAR` afterward).

ENVIRONMENT NOTE: this sandbox had no Python dependencies installed at all
(fresh container — `numpy`, `pandas`, `lightgbm`, etc. all missing, plus
`openpyxl` for an unrelated existing test file). Ran `pip install -r
requirements.txt` + `pip install openpyxl` before any test could execute;
worth flagging in case future autonomous sessions hit the same cold-start
in a fresh container and mistake "ModuleNotFoundError" for a code bug.

GATES: `python3 -m pytest -q` — 626 passed, 2 skipped (baseline 621 + 5
new; zero regressions across the full suite including the silent-except
ratchet after the narrowing fix). No TypeScript/client files touched, so
no `tsc`/`npm run build`/visual harness run (nothing in scope for those
gates). No backtest run — per PROMOTION RULES this is an execution-layer
safety fix (whether an order may be submitted, not what to trade or how
much), the same category KNOWN BROKEN #8 shipped in without a backtest;
no scoring, sizing, or signal logic changed.

Version 1.0.262 -> 1.0.263 (read-and-incremented at commit time).

LIVE VERIFICATION STILL PENDING: this session cannot deploy or observe
Railway directly. NEXT for the first session after this deploys: query
`/api/diag/orders?token=$DIAG_TOKEN` and confirm no new SMH (or any other
`FLOOR_BASKET` member) market orders appear during extended hours from
this point forward. Also worth checking whether `_manage_spy_floor`'s
`*_deferred` actions are visible anywhere in the audit trail —
`spy_floor_result` is returned from `scan_market()` but a quick grep
didn't show it being `audit()`-logged in `server/bot.ts` the way
`TIER3-*`/`DIAGNOSTIC`/etc. events are; if the deferrals turn out to be
invisible in production, that's a small follow-up visibility gap (not a
reopening of this fix — the deferrals are still correctly happening,
just not surfaced) worth a KNOWN BROKEN entry of its own once confirmed.

MARKET-HOURS NOTE: session ran pre-market ET (~07:00 ET, well before the
9:30am open). This PR touches live order-submission code
(`bot_engine.py`), unlike most recent PRODUCT sessions — but the change is
strictly risk-reducing (it can only prevent an order from being submitted
that the current code would otherwise send doomed into extended hours; it
never adds a new order-submission path or changes sizing/target logic), and
the bug it fixes is actively firing right now, so deferring the fix until
after the open would mean tolerating more of the exact waste this PR
stops. Merging pre-open is judged lower-risk than waiting.
