...
def _select_sell_put(contracts: list, price: float, equity: float, ticker: str, size_pct: float = 0.05,
    ...
    if not affordable_puts:
        # BUG FIX 2026-08-12: `puts` (strike <= price candidates) can itself
        # be empty — e.g. the fetched chain simply has no OTM/ATM puts for
        # this ticker. The old code let `min(..., default=0)` silently paper
        # over that case, producing the nonsensical "smallest strike $0
        # needs $0 ... Underlying too expensive" message (seen live on KORU/
        # HYG in /api/diag/audit type=T2-FAIL). Distinguish "no candidate
        # puts in the chain" from "cheapest candidate still unaffordable" —
