...
WHY THIS EXISTS (gap found live, 2026-07-10, gv-div4-ks): runpod_launch.py's
OPTION A CAVEAT (research/runpod_ledger.md, human-accepted 2026-07-08) is that
the cost-cap watchdog lives in the launching Claude Code session. This repo's
remote sessions run in an EPHEMERAL container that is reclaimed when the
session ends — so if a session ends (or is interrupted) before its watchdog
loop reaches the terminate step, NOTHING is left watching that pod. Confirmed
live: gv-div4-ks's training crashed in under 10 minutes (dataset bug, see
train.py's compute_holdout_split), but the pod itself stayed `RUNNING` and
billing for 2h13m afterward — with zero live process watching it — until this
script's manual precursor found and terminated it. The in-pod `timeout`
wrapper does NOT help here: it bounds the training PROCESS, not the pod's
billing (a pod bills until DELETEd, confirmed live — exiting its start command
