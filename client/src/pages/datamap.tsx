          ...
          {(enabled.sites || enabled.powerplants || enabled.powergrid_hifld_plants || enabled.powergrid_hifld_sub || enabled.plant_operations || enabled.nrc_reactor_status || enabled.faa_airports || enabled.border_waits || enabled.coal_terminals || enabled.iron_ore_mines || enabled.iron_steel_plants || enabled.chemicals || enabled.lng_shipyards || enabled.steel_raw_materials) && (
            <div className="vt-legend-sec">
              ...
              <div className="vt-legend-items">
                ...
                {enabled.chemicals && (
                  <>
                    {Object.keys(CHEMICAL_FEEDSTOCK_LABEL).map((f) => (
          ...
          {(enabled.fires || enabled.surfacewater || enabled.forest || enabled.nightlights || enabled.aerosol || enabled.vegetation || enabled.soilmoisture || enabled.no2 || enabled.so2 || enabled.floods || enabled.firetemp || enabled.biomass || enabled.rivergauges || enabled.alerts || enabled.spaceweather || enabled.meteors || enabled.earthquakes || enabled.volcanoes || enabled.buoys || enabled.methane_plumes || enabled.coal_mine_features) && (
            <div className="vt-legend-sec">
              ...
              <div className="vt-legend-items">
                ...
                {enabled.so2 && (
                  <>
                    <span className="vt-legend-chip"><i style={{ background: "#9c5fd6" }} /> SO₂ (OMPS, planetary boundary layer)</span>
                    <span className="vt-legend-note">(daily, NASA GIBS/OMPS-NOAA20 — {so2Date})</span>
                  </>
                )}
                {enabled.floods && (
                  <>
...
export default function DataMapPage() {
  ...
  // FDA binary events (#/data/fda-events) — same overlay pattern
  // (server/fdaEvents.ts, DATA STREAM EXPANSION stream #5, shipped
  ...
  // open_questions.md 2026-07-27 "NOT PREVIOUSLY FILED" C.1: SO2 column
  // (OMPS-NOAA20, planetary boundary layer — nearest-surface of the three
  // altitude bands GIBS offers, most relevant to volcanic degassing and
  // industrial/smelter plumes). Same daily/~1-day-lag pattern as NO2.
  const [so2Date, setSo2Date] = useState<string>(() => gibsDefaultDate(Date.now()));
  ...
  useEffect(() => {
    const apply = () => {
      ...
      setSo2Date(gibsDateForAxis(axis, now).dateISO);
  ...
  // ── sulfur dioxide column (RAW; open_questions.md 2026-07-27 "NOT
  // PREVIOUSLY FILED" C.1 — NASA GIBS OMPS-NOAA20 planetary-boundary-layer
  // SO2 via the shared gibs.ts factory, the same DATED-daily pattern as
  // NO2/aerosol above). Access + non-blank field verified live 2026-08-01
  // (six z=2 tiles sampled globally, distinct byte sizes/hashes — a real
  // field, not a placeholder). PBL (nearest-surface of GIBS's three SO2
  // altitude bands: Lower/Middle/PBL) is the band most relevant to the
  // filed cross-ties: volcanic degassing (vs. USGS volcano alert levels,
  // still unbuilt) and industrial/smelter/refinery plumes (vs. powerplants
  // + coal_mine_features). As-is display only — no predictive claim; both
  // cross-ties remain open gate-2 hypotheses. ──
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !mapReady) return;
    if (!enabled.so2) {
      try {
        if (map.getLayer("gibs-so2")) map.removeLayer("gibs-so2");
        if (map.getSource("gibs-so2")) map.removeSource("gibs-so2");
      } catch {}
      setStatus("so2", "off");
      return;
    }
    try {
      ...
      if (map.getSource("gibs-so2")) map.removeSource("gibs-so2");
      const url = gibsTileUrl(
        { layer: "OMPS_NOAA20_SO2_Planetary_Boundary_Layer", tileMatrixSet: "GoogleMapsCompatible_Level6", ext: "png" },
        so2Date,
      );
      map.addSource("gibs-so2", {
        type: "raster", tiles: [url], tileSize: 256, maxzoom: 6,
        attribution: "Sulfur dioxide column (planetary boundary layer) · Suomi NPP/NOAA-20 OMPS · NASA GIBS/ESDIS (public domain)",
      } as any);
      const firstMarker = (map.getStyle().layers || []).find((l: any) => ["symbol", "circle", "line"].includes(l.type));
      map.addLayer({
        id: "gibs-so2", type: "raster", source: "gibs-so2",
        paint: { "raster-opacity": opacityOf("so2") / 100 },
      } as any, firstMarker?.id);
      setStatus("so2", "active", undefined,
        `SO₂ column (planetary boundary layer) for ${so2Date} (UTC) · OMPS-NOAA20 via NASA GIBS/ESDIS — ` +
        `purpler = more SO₂ (volcanic degassing, smelters/refineries); satellite swath gaps and cloud can leave stripes/blanks`);
    } catch {
      setStatus("so2", "error");
    }
  }, [enabled.so2, so2Date, mapReady, setStatus]);
...
// deep-space population (GEO comms,
  ...
  // ── GEM chemical plants (RAW; server/gemChemicals.ts) — 868 plants
  // worldwide. New "vt-flask" glyph (a chemical plant is a genuinely
  // different real-world kind from the mill/mine/terminal symbols above,
  // per SYMBOLS NOT DOTS — no existing glyph fit); color = catalogued
  // primary feedstock family (coal/natural_gas/petroleum/ngl/low_carbon/
  // other) — this release's own lifecycle-status-free dimension, same as
  // iron_steel_plants. Static reference dataset, same Law-I-compliant
  // toggle-on/off mount pattern as coal_terminals/iron_ore_mines/
  // iron_steel_plants above. ──
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !mapReady) return;
    const clear = () => {
      try {
        if (map.getLayer("chemicals-pt")) map.removeLayer("chemicals-pt");
        if (map.getSource("chemicals-points")) map.removeSource("chemicals-points");
      } catch {}
    };
    if (!enabled.chemicals) { clear(); setStatus("chemicals", "off"); return; }
    if (!mapSettled) { setStatus("chemicals", "loading", undefined, "queued — mounts after the map settles"); return; }
    setStatus("chemicals", "loading");
    let detach = () => {};
    const stopLoad = runResilientLoad(
      async (signal) => {
        const r = await fetch("/api/data/chemicals", { signal });
        if (!r.ok) throw new Error(String(r.status));
        const d = await r.json();
        if (signal.aborted || !Array.isArray(d.plants)) throw new Error("no plants");
        if (map.getSource("chemicals-points")) return;
        map.addSource("chemicals-points", {
          type: "geojson",
          data: {
            type: "FeatureCollection",
            features: d.plants.map((p: any) => ({
              type: "Feature",
              geometry: { type: "Point", coordinates: [p.lon, p.lat] },
              properties: { ...p, tint: chemicalFeedstockColor(p.feedstockFamily), icon: "vt-flask" },
            })),
          } as any,
          attribution: "Global Energy Monitor (CC BY 4.0)",
        } as any);
        map.addLayer({
          id: "chemicals-pt", type: "symbol", source: "chemicals-points",
          layout: {
            "icon-image": ["get", "icon"],
            "icon-size": ["interpolate", ["linear"], ["zoom"], 2, 0.3, 8, 0.55],
            "icon-allow-overlap": false,
        ...
        const onClick = (e: any) => {
          const f = e.features?.[0]; if (!f) return; const p = f.properties;
          const familyLabel = CHEMICAL_FEEDSTOCK_LABEL[p.feedstockFamily] || p.feedstockFamily;
          setDetail({
            kind: "chemicalplant",
            title: p.name,
            subtitle: familyLabel,
            stats: [
              { label: "Feedstock family", value: familyLabel },
              { label: "Primary products", value: p.primaryProducts || "not stated" },
              { label: "Country", value: p.country || "—" },
              { label: "Owner", value: p.owner || "—" },
            ],
            sourceTag: "GEM CC BY 4.0",
            body: `${p.secondaryProducts ? `Secondary products: ${p.secondaryProducts}\n` : ""}` +
                  `Feedstock (as catalogued): ${p.feedstockRaw || "not stated"}\n` +
                  `Coordinate accuracy: ${p.coordinateAccuracy || "not stated"}\n\n` +
                  `Source: Global Energy Monitor — Global Chemicals Inventory (CC BY 4.0). ` +
                  `Location/feedstock as catalogued; no forecast, valuation, or trading signal.`,
            sourceUrl: p.wiki || undefined,
          });
        };
        detach = attachLayerInteractions(map, "chemicals-pt", onClick);
        setStatus("chemicals", "active", d.count,
          `${d.count.toLocaleString()} plants — Global Energy Monitor CC BY 4.0${d.release ? `, release ${d.release}` : ""}`);
      },
      (failures) => setStatus("chemicals", "error", undefined,
        failures === 0 ? "load failed — retrying automatically…" : "still retrying automatically…"),
    );
    return () => { stopLoad(); detach(); };
  ...
  // ── FDA binary events (RAW; non-geospatial — same inline-panel-row +
  // full-view pattern as treasury_dts/treasury_auctions/tff). Server
  // refreshes on a 6h poll (both openFDA and Federal Register publish on
  // daily-or-slower cycles), so this poll only refreshes the panel's
  // event-count badge, same 300s convention as the sibling filings
  // layers. ──
  useEffect(() => {
    if (!enabled.fda_events) { setStatus("fda_events", "off"); return; }
    if (!mapSettled) { setStatus("fda_events", "loading", undefined, "queued — mounts after the map settles"); return; }
    setStatus("fda_events", "loading");
    let stop = false;
    const load = async () => {
      try {
        ...
        const d = await r.json();
        if (stop) return;
        if (d.warming_up) { setStatus("fda_events", "loading", 0, "warming up — first poll can take a minute"); return; }
        setStatus("fda_events", "active", d.count);
      } catch {
        if (!stop) setStatus("fda_events", "error", undefined, "feed error — retrying");
      }
    };
    load();
    const iv = window.setInterval(() => { if (!document.hidden) load(); }, 300_000);
    return () => { stop = true; window.clearInterval(iv); };
  }, [enabled.fda_events, mapSettled, setStatus]);

  // ── NHTSA vehicle complaints (RAW; non-geospatial — same inline-panel-
  // row + full-view pattern as fda_events/treasury_dts/treasury_auctions/
  // tff). Server refreshes on a 12h poll (bulk file updates daily), so
  // this poll only refreshes the panel's vehicle-count badge, same 300s
  // convention as the sibling filings layers. ──
  useEffect(() => {
    if (!enabled.vehicle_complaints) { setStatus("vehicle_complaints", "off"); return; }
    if (!mapSettled) { setStatus("vehicle_complaints", "loading", undefined, "queued — mounts after the map settles"); return; }
    setStatus("vehicle_complaints", "loading");
    let stop = false;
    const load = async () => {
      try {
        ...
        const d = await r.json();
        if (stop) return;
        if (d.warming_up) { setStatus("vehicle_complaints", "loading", 0, "warming up — first poll can take a minute"); return; }
        setStatus("vehicle_complaints", "active", d.count);
      } catch {
        if (!stop) setStatus("vehicle_complaints", "error", undefined, "feed error — retrying");
      }
    };
    load();
    const iv = window.setInterval(() => { if (!document.hidden) load(); }, 300_000);
  ...
  // ── USAspending.gov federal contract awards (RAW; non-geospatial — same
  // inline-panel-row + full-view pattern as bank_failures/fleet_utilization
  // above). Server refreshes on a 6h poll; this poll only refreshes the
  // panel's award-count badge, same 300s convention as the sibling filings
  // layers. GATE 2's award/mcap-ratio hypothesis was REJECTED 2026-08-15 —
  // RAW display only, never surfaced as a signal (usaspendingContracts.tsx
  // carries the full honesty account). ──
  useEffect(() => {
    if (!enabled.contracts) { setStatus("contracts", "off"); return; }
    if (!mapSettled) { setStatus("contracts", "loading", undefined, "queued — mounts after the map settles"); return; }
    ...
    const load = async () => {
      try {
        ...
        const d = await r.json();
        if (stop) return;
        if (d.warming_up) { setStatus("contracts", "loading", 0, "warming up — first poll can take a minute"); return; }
        setStatus("contracts", "active", d.count);
      } catch {
        if (!stop) setStatus("contracts", "error", undefined, "feed error — retrying");
      }
    };
    load();
    const iv = window.setInterval(() => { if (!document.hidden) load(); }, 300_000);
    return () => { stop = true; window.clearInterval(iv); };
  }, [enabled.contracts, mapSettled, setStatus]);
  ...
  const layerIcon = (id: string) =>
    ...
    id === "contracts" ? <Handshake size={15} /> :
      ...
      <div key={l.id}>
        ...
        {(l as any).field && on && (
          <div className="vt-field-controls" role="group" aria-label={`${l.name} display controls`}>
            ...
            {l.id === "so2" && (
              <div className="vt-gibs-scrubber" role="group" aria-label="SO2 date">
                <button
                  aria-label="Previous day"
                  onClick={() => setSo2Date((d) => gibsStepDate(d, -1))}
                >
                  <ChevronLeft size={13} />
                </button>
                <span className="vt-gibs-scrubber-date">{so2Date} UTC</span>
                <button
                  aria-label="Next day"
                  disabled={gibsIsLatestAvailable(so2Date, Date.now())}
                  onClick={() => setSo2Date((d) => gibsStepDate(d, 1))}
                >
                  <ChevronRight size={13} />
                </button>
        ...
        {l.id === "fda_events" && on && (
          // Same pattern as treasury_dts/treasury_auctions/tff: an
          // approvals + advisory-committee event listing doesn't belong
          // in a layer-toggle sidebar.
          <div style={{ padding: "0 14px" }}>
            <button className="vt-filings-openfull"
                    onClick={() => { window.location.hash = "#/data/fda-events"; setFdaEventsOpen(true); }}>
              Open FDA binary events view — approvals &amp; advisory-committee meetings →
            </button>
          </div>
        )}
        {l.id === "vehicle_complaints" && on && (
          // Same pattern as fda_events/treasury_dts/treasury_auctions/tff:
          // a per-vehicle complaint count table doesn't belong in a
          // layer-toggle sidebar.
          <div style={{ padding: "0 14px" }}>
            <button className="vt-filings-openfull"
                    onClick={() => { window.location.hash = "#/data/vehicle-complaints"; setVehicleComplaintsOpen(true); }}>
              Open vehicle complaints view — per-ticker complaint counts →
            </button>
          </div>
        )}
        ...
        {l.id === "contracts" && on && (
          // Same pattern as bank_failures/fleet_utilization/tff: a ranked
          // contract-award listing doesn't belong in a layer-toggle sidebar.
          <div style={{ padding: "0 14px" }}>
            <button className="vt-filings-openfull"
                    onClick={() => { window.location.hash = "#/data/contracts"; setContractsOpen(true); }}>
              Open contract awards view — ranked by amount →
            </button>
          </div>
        )}
        {l.id === "attention" && on && (
  ...
  return (
    <div
      ...
      {contractsOpen && (
        <UsaspendingContractsView onBack={() => { window.location.hash = "#/data"; setContractsOpen(false); }} />
      )}
      {attentionOpen && (
