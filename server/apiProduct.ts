...
export const LICENSE_MARKS: Record<string, { license: string; attribution: string; resell: "ok" | "share-alike" | "conditional" }> = {
  ...
  "data/coal-terminals": {
    license: "Global Energy Monitor — Global Coal Terminals Tracker (521 port coal-handling terminals worldwide, lifecycle status + terminal type + stated capacity, as catalogued by GEM) — GEM publishes this release under CC BY 4.0, the same open-attribution class as the methane-plumes/CFTC-COT/USAspending/FRED/crop-conditions/bank-failures/NRC/attention/Digitraffic-trains streams above, NOT conditional like the issuer-authored Form 4/13F/earnings-language/DTCC streams or the informational-use-terms OCC/Cboe/FINRA streams.",
    attribution: "Global Energy Monitor — Global Coal Terminals Tracker (CC BY 4.0)",
    resell: "ok",
  },
  "data/coal-mine-features": {
    license: "Global Energy Monitor — Coal Mine Boundaries and Methane Sources (mine boundary polygons, ventilation/degasification points, and other catalogued mine infrastructure, as researched and mapped by GEM) — GEM publishes this release under CC BY 4.0, the same open-attribution class as the coal-terminals/methane-plumes/CFTC-COT/USAspending/FRED/crop-conditions/bank-failures/NRC/attention/Digitraffic-trains streams above, NOT conditional like the issuer-authored Form 4/13F/earnings-language/DTCC streams or the informational-use-terms OCC/Cboe/FINRA streams.",
    attribution: "Global Energy Monitor — Coal Mine Boundaries and Methane Sources (CC BY 4.0)",
    resell: "ok",
  },
  "data/iron-ore-mines": {
    license: "Global Energy Monitor — Global Iron Ore Mines Tracker (949 iron ore mines worldwide, lifecycle operating status + stated 2022-2024 production + design capacity + reserve/resource tonnage, as catalogued by GEM) — GEM publishes this release under CC BY 4.0, the same open-attribution class as the coal-terminals/coal-mine-features/methane-plumes/CFTC-COT/USAspending/FRED/crop-conditions/bank-failures/NRC/attention/Digitraffic-trains streams above, NOT conditional like the issuer-authored Form 4/13F/earnings-language/DTCC streams or the informational-use-terms OCC/Cboe/FINRA streams.",
  ...
  "data/iron-steel-plants": {
    license: "Global Energy Monitor — Global Iron and Steel Tracker (1,293 iron and steel plants worldwide, primary production technology + product category + lifecycle dates, as catalogued by GEM, each plant enriched with its individual furnace units — type/status/capacity/dates — from GEM's companion steel-unit-level release) — GEM publishes this release under CC BY 4.0, the same open-attribution class as the coal-terminals/coal-mine-features/iron-ore-mines/chemicals/methane-plumes/CFTC-COT/USAspending/FRED/crop-conditions/bank-failures/NRC/attention/Digitraffic-trains streams above, NOT conditional like the issuer-authored Form 4/13F/earnings-language/DTCC streams or the informational-use-terms OCC/Cboe/FINRA streams.",
  ...
  "data/steel-raw-materials": {
    license: "Global Energy Monitor — Production-Consumption of Met Coal & Iron Ore by Steel Industry (national met-coal/iron-ore mining and pig-iron/DRI production-consumption balance, one row per country, as catalogued by GEM) — GEM publishes this release under CC BY 4.0, the same open-attribution class as the coal-terminals/coal-mine-features/iron-ore-mines/chemicals/iron-steel-plants/lng-shipyards/methane-plumes/CFTC-COT/USAspending/FRED/crop-conditions/bank-failures/NRC/attention/Digitraffic-trains streams above, NOT conditional like the issuer-authored Form 4/13F/earnings-language/DTCC streams or the informational-use-terms OCC/Cboe/FINRA streams.",
    attribution: "Global Energy Monitor — Production-Consumption of Met Coal & Iron Ore by Steel Industry (CC BY 4.0)",
    resell: "ok",
  },
...
export function apiMeta() {
  return {
    ...
    endpoints: [
      ...
      { path: "/api/v1/data/crop-conditions-history", params: "?weeks<=104 (default 52), ?commodity=CORN|SOYBEANS (optional)", desc: "Accumulated USDA NASS crop-condition archive — the multi-week companion to /api/v1/data/crop-conditions above, which mirrors only the latest poll cache. No commodity: both CORN and SOYBEANS pivoted to {class: pct} per week, the shape a condition-DELTA gate-2 test (week-over-week change vs forward grain futures returns) needs directly. ?commodity=CORN|SOYBEANS: that commodity's own raw weekly rows (all 5 classes), read from a full archive scan (a given week's row can land in any day's file, since the underlying QuickStats call requests the whole season on every poll). Same GATE 1 PASS / GATE 2 NOT-ATTEMPTED status and the same public-domain-resell posture as data/crop-conditions (not a separate root, not a separate license); also requires NASS_API_KEY, returns 503 if not configured.", preview: "/api/data/crop-conditions/history" },
      ...
      { path: "/api/v1/data/steel-raw-materials", params: "-", desc: "Global Energy Monitor Production-Consumption of Met Coal & Iron Ore by Steel Industry: national met-coal/iron-ore mining and pig-iron/DRI production-consumption balance, one row per country. A country-level ACCOUNTING STATISTIC, not a forecast, valuation, or trading signal — not wired into any ladder gate (global_energy_monitor is a raw_only root, datacore/signal_ladder.json). GENUINELY DIFFERENT SHAPE from the rest of the GEM family: no per-row coordinates, so this endpoint also returns a country-choropleth GeoJSON (joined by name onto the vendored Natural Earth 1:110m admin0 boundaries) alongside the flat balance rows; countries with a polygon but no GEM record render as 'no data', never a false zero. GEM publishes this release under CC BY 4.0, freely resellable with attribution.", preview: "/api/data/steel-raw-materials" },
...
export function agentToolSpec(baseUrl = "https://voltradeai.com") {
  const tools = [
    ...
    {
      name: "voltrade_crop_conditions_history",
      description: "Accumulated USDA NASS crop-condition archive — the multi-week companion to voltrade_crop_conditions above, which returns only the latest poll cache. Same GATE 1 (DATA) PASSED / GATE 2 NOT-ATTEMPTED status, and the same public-domain resell posture as voltrade_crop_conditions — not a separate root or a separate license. No commodity param: both CORN and SOYBEANS pivoted to {class: pct} per week, ascending — the shape a condition-DELTA gate-2 test needs directly. commodity param: that commodity's own raw weekly rows (all 5 classes), read from a full archive scan (a given week's row can land in any day's file, since the underlying QuickStats call requests the whole season on every poll).",
      input_schema: {
        type: "object",
        properties: {
          weeks: { type: "integer", minimum: 1, maximum: 104, default: 52, description: "Lookback window in weeks (max 104)." },
          commodity: { type: "string", enum: ["CORN", "SOYBEANS"], description: "Optional — returns that commodity's own raw weekly rows instead of the pivoted both-commodity trend." },
        },
        required: [],
      },
      endpoint: "GET /api/v1/data/crop-conditions-history?weeks={weeks}&commodity={commodity}",
      returns_provenance: ["data/crop-conditions"],
    },
    {
    ...
    {
      name: "voltrade_coal_terminals",
      description: "Global Energy Monitor Global Coal Terminals Tracker: 521 port coal-handling terminals worldwide, each carrying a lifecycle status (Operating/Construction/Proposed/Shelved/Mothballed/Retired/Cancelled), a classified terminal-role bucket (exports/imports/domestic/mixed/unstated — derived by substring-testing GEM's free-text Terminal Type column, NEVER inferring a role the source string does not state), stated capacity (Mt), owner, parent port, and country/region. RAW catalogued display, no predictive claim — this is reference infrastructure data, not wired into any ladder gate (global_energy_monitor is a raw_only root, current_gate 0). GEM publishes this release under CC BY 4.0 — freely resellable with attribution, same posture as the CFTC COT/USAspending/FRED/crop-conditions/bank-failures/NRC/attention/methane-plume tools above.",
      input_schema: { type: "object", properties: {}, required: [] },
      endpoint: "GET /api/v1/data/coal-terminals",
      returns_provenance: ["data/coal-terminals"],
    },
    {
      name: "voltrade_coal_mine_features",
      description: "Global Energy Monitor Coal Mine Boundaries and Methane Sources: catalogued mine boundary polygons, ventilation/degasification points, and other mapped mine infrastructure, each carrying GEM's own per-feature id, a feature category/subcategory, a short description, mine name/id, owners, parent company, country, coal grade, and GEM wiki link where GEM states one — rows with no id or no geometry are dropped, never guessed. RAW catalogued geometry, no predictive claim — this is reference infrastructure data, not wired into any ladder gate (global_energy_monitor is a raw_only root, current_gate 0). GEM publishes this release under CC BY 4.0 — freely resellable with attribution, same posture as the coal-terminals/CFTC COT/USAspending/FRED/crop-conditions/bank-failures/NRC/attention/methane-plume tools above.",
      input_schema: { type: "object", properties: {}, required: [] },
      endpoint: "GET /api/v1/data/coal-mine-features",
      returns_provenance: ["data/coal-mine-features"],
    },
    {
      name: "voltrade_iron_ore_mines",
      description: "Global Energy Monitor Global Iron Ore Mines Tracker: 949 iron ore mines worldwide, each carrying a lowercase lifecycle operating-status bucket (operating/proposed/mothballed/retired/shelved/cancelled/unknown — a value outside this set is never guessed, it degrades to 'unknown' honestly), stated production for 2022/2023/2024 (thousand tonnes/year), design capacity, total reserves (proven and probable) and total resource (inferred, indicated and measured) tonnage, start/stop dates, owner, parent, and GEM wiki link where GEM states one. RAW catalogued display, no predictive claim — this is reference infrastructure data, not wired into any ladder gate (global_energy_monitor is a raw_only root, current_gate 0). GEM publishes this release under CC BY 4.0 — freely resellable with attribution, same posture as the coal-terminals/coal-mine-features/CFTC COT/USAspending/FRED/crop-conditions/bank-failures/NRC/attention/methane-plume tools above.",
      input_schema: { type: "object", properties: {}, required: [] },
      endpoint: "GET /api/v1/data/iron-ore-mines",
    ...
    {
      name: "voltrade_iron_steel_plants",
      description: "Global Energy Monitor Global Iron and Steel Tracker: 1,293 iron and steel plants worldwide, each carrying a classified primary production-technology bucket (bf_bof/dri/eaf/if/other — derived by presence-priority over GEM's free-text semicolon-separated Main production equipment column, NEVER inferring a technology the row doesn't state), product category, workforce size, start/retired/idled dates, owner, parent, SOE status, and country/region/municipality. Each plant's `units` array carries its individual furnaces (joined by plant id from GEM's companion steel-unit-level release, 1,210 of 1,293 plants / 3,554 units total): furnace type (eaf/bof/induction/open_hearth), an 8-value catalogued lifecycle status, current capacity (ttpa), dates, and manufacturer. RAW catalogued display, no predictive claim — this is reference infrastructure data, not wired into any ladder gate (global_energy_monitor is a raw_only root, current_gate 0). GEM publishes this release under CC BY 4.0 — freely resellable with attribution, same posture as the coal-terminals/coal-mine-features/iron-ore-mines/chemicals/CFTC COT/USAspending/FRED/crop-conditions/bank-failures/NRC/attention/methane-plume tools above.",
      input_schema: { type: "object", properties: {}, required: [] },
      endpoint: "GET /api/v1/data/iron-steel-plants",
    ...
    {
      name: "voltrade_steel_raw_materials",
      description: "Global Energy Monitor Production-Consumption of Met Coal & Iron Ore by Steel Industry: national met-coal/iron-ore mining and pig-iron/DRI production-consumption balance, one row per country (met coal mined, iron ore mined, met coal/iron ore consumed by pig-iron vs. DRI route, pig iron and DRI produced — all in thousand tonnes/year, ttpa). RAW catalogued country-level ACCOUNTING STATISTIC, no predictive claim — this is reference infrastructure data, not wired into any ladder gate (global_energy_monitor is a raw_only root, current_gate 0). GENUINELY DIFFERENT SHAPE from the rest of the GEM family: no per-facility coordinates, so this tool also returns a country-choropleth GeoJSON (joined by name onto the vendored Natural Earth 1:110m admin0 boundaries); a reported 0 is GEM's own figure and is kept distinct from an unreported country (has_data:false), never conflated. GEM publishes this release under CC BY 4.0 — freely resellable with attribution, same posture as the coal-terminals/coal-mine-features/iron-ore-mines/chemicals/iron-steel-plants/lng-shipyards/CFTC COT/USAspending/FRED/crop-conditions/bank-failures/NRC/attention/methane-plume tools above.",
      input_schema: { type: "object", properties: {}, required: [] },
      endpoint: "GET /api/v1/data/steel-raw-materials",
      returns_provenance: ["data/steel-raw-materials"],
    },
    {
...
export const RESPONSE_DATA_SCHEMAS: Record<string, Record<string, unknown>> = {
  ...
  // branches on ?commodity= (single-commodity raw rows) vs. no query (both-commodity pivoted trend).
  voltrade_crop_conditions_history: dataObj({ commodity: STR, weeks: INT, count: INT, rows: ARR, trend: ARR }),
  ...
  // server/gemCoalTerminals.ts's CoalTerminalsResult: a static reference
  // registry (no warming_up cache-miss state once datacore/gem/coal_terminals.json
  // is checked in — same shape family as voltrade_jodi_oil_stocks/voltrade_un_comtrade
  // below, not the poller-backed voltrade_methane_plumes/voltrade_dtcc_swaps above).
  voltrade_coal_terminals: dataObj({
    count: INT, attribution: STR, license: STR, release: ANY, note: STR, terminals: ARR,
  }, ["count", "attribution", "license", "note", "terminals"]),
  // server/gemCoalMineFeatures.ts's CoalMineFeaturesResult: same static
  // reference shape family as voltrade_coal_terminals above (release read
  // off hit.buildVersion, attribution/license hardcoded since the source
  // file carries no top-level provenance object — see the module's own
  // header comment), features carrying raw GeoJSON geometry.
  voltrade_coal_mine_features: dataObj({
    count: INT, attribution: STR, license: STR, release: ANY, note: STR, features: ARR,
  }, ["count", "attribution", "license", "note", "features"]),
  // server/gemIronOreMines.ts's IronOreMinesResult/IronOreMine: same static
  // reference shape family as voltrade_coal_terminals/voltrade_coal_mine_
  // features above. Each mine row's own field list read directly off the
  // IronOreMine interface (id/name/status/statusRaw required by
  // normalizeIronOreMines' own drop-not-infer rule; every tonnage/date/
  // owner/parent/wiki field is `| null`, so ANY not required).
  voltrade_iron_ore_mines: dataObj({
    count: INT, attribution: STR, license: STR, release: ANY, note: STR,
    mines: { type: "array", items: dataObj({
      id: STR, name: STR, status: STR, statusRaw: ANY, country: ANY, region: ANY, municipality: ANY,
      subnationalUnit: ANY, coordinateAccuracy: ANY, production2024Kt: ANY, production2023Kt: ANY,
      production2022Kt: ANY, designCapacityKt: ANY, totalReservesKt: ANY, totalResourceKt: ANY,
      startDate: ANY, stopDate: ANY, owner: ANY, parent: ANY, wiki: ANY,
  ...
  // server/gemIronSteelPlants.ts's IronSteelPlantsResult/IronSteelPlant: same
  // static reference shape family as voltrade_coal_terminals/voltrade_coal_
  // mine_features/voltrade_iron_ore_mines/voltrade_chemicals above. Each
  // plant row's own field list read directly off the IronSteelPlant
  // interface (id/name/technology required by normalizeIronSteelPlants' own
  // drop-not-infer rule; every other product/location/owner/date field is
  // `| null`, so ANY not required).
  voltrade_iron_steel_plants: dataObj({
    count: INT, attribution: STR, license: STR, release: ANY, note: STR,
    plants: { type: "array", items: dataObj({
      id: STR, name: STR, technology: STR, technologyRaw: ANY, categorySteelProduct: ANY,
      country: ANY, region: ANY, municipality: ANY, subnationalUnit: ANY, coordinateAccuracy: ANY,
      workforceSize: ANY, startDate: ANY, retiredDate: ANY, idledDate: ANY, owner: ANY, parent: ANY,
      soeStatus: ANY, wiki: ANY,
      lat: { type: "number" }, lon: { type: "number" },
      // Furnace-level detail joined from steel_units.json, keyed by plant id
      // (server/gemIronSteelPlants.ts's SteelFurnaceUnit) — [] when none
      // joined, never absent, so a schema-consuming client never needs an
      // existence check.
      units: { type: "array", items: dataObj({
        plantId: STR, unitId: STR, name: ANY, furnaceType: STR, status: STR,
        capacityTtpa: ANY, startDate: ANY, retiredDate: ANY, manufacturer: ANY,
      }, ["plantId", "unitId", "furnaceType", "status"]) },
    }, ["id", "name", "technology", "lat", "lon", "units"]) },
  }, ["count", "attribution", "license", "note", "plants"]),
  ...
  // server/gemSteelRawMaterials.ts's CountryBalanceResult — a COUNTRY-level
  // accounting balance sheet, not a per-facility row (see the module's own
  // header comment). Every CountryBalance field beyond `country` is
  // `| null` per the module's own drop-not-infer rule (GEM's "unknown"
  // sentinel and any non-numeric value degrade to null, never guessed).
  // `geojson` carries the country-choropleth join (admin0 FeatureCollection,
  // `has_data`/ttpa fields on each feature's properties); `matchedCountries`/
  // `unmatchedCountries` are the join's own counts, same as the RAW route.
  voltrade_steel_raw_materials: dataObj({
    count: INT, attribution: STR, license: STR, release: ANY, note: STR,
    balances: { type: "array", items: dataObj({
      country: STR, metCoalMinedTtpa: ANY, ironOreMinedTtpa: ANY,
      metCoalConsumedPigIronTtpa: ANY, ironOreConsumedPigIronTtpa: ANY,
      ironOreConsumedDriTtpa: ANY, ironOreConsumedTotalTtpa: ANY,
      pigIronProducedTtpa: ANY, driProducedTtpa: ANY,
    }, ["country"]) },
    geojson: ANY, matched_countries: INT, unmatched_countries: INT,
  }, ["count", "attribution", "license", "note", "balances"]),
