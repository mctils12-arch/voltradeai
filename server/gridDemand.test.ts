...
test("url: brackets encoded, key encoded, demand-only, bounded window", () => {
  ...
  assert.ok(u.includes("facets%5Btype%5D%5B%5D=D"), "demand series only");
  assert.ok(u.includes(`length=${HOURS_PER_FETCH}`));
});

test("parseDemand: string MWh -> number, bad periods dropped, non-D dropped", () => {
...
test("refresh sweep: one call per respondent, per-respondent stats cached", async () => {
  ...
  const ok = async (url: string) => {
    ...
    return { ok: true, status: 200,
             text: async () => JSON.stringify(ENVELOPE([ROW("2026-07-06T21", resp, "100"), ROW("2026-07-06T20", resp, "90")])) };
  };
