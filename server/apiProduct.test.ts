...
test("meta honesty: gated products listed as coming, never as live endpoints; Graph v1 now IS live", () => {
  ...
  assert.ok(paths.includes("/api/v1/data/earnings-language-history"), "SEC 8-K earnings-language accumulated filing-history keyed mirror shipped — must be a live endpoint");
  assert.ok(paths.includes("/api/v1/data/appstore-rankings"), "App Store rankings keyed mirror shipped — must be a live endpoint");
  assert.ok(paths.includes("/api/v1/data/appstore-rankings-history"), "App Store rankings accumulated history keyed mirror shipped — must be a live endpoint");
  ...
  assert.ok(paths.includes("/api/v1/data/github-activity-history"), "GitHub org engineering-momentum accumulated history keyed mirror shipped — must be a live endpoint");
  ...
  assert.ok(paths.includes("/api/v1/data/13f-holdings-history"), "SEC 13F-HR accumulated filing-history keyed mirror shipped — must be a live endpoint");
  ...
  assert.ok(paths.includes("/api/v1/data/coal-terminals"), "GEM Global Coal Terminals Tracker keyed mirror shipped — must be a live endpoint");
  assert.ok(paths.includes("/api/v1/data/coal-mine-features"), "GEM Coal Mine Boundaries and Methane Sources keyed mirror shipped — must be a live endpoint");
  ...
  assert.ok(paths.includes("/api/v1/data/steel-raw-materials"), "GEM Production-Consumption of Met Coal & Iron Ore by Steel Industry keyed mirror shipped — must be a live endpoint");
...
test("appstore-rankings-history (accumulated App Store archive) shares the appstore-rankings license mark and gate status — not a separate root, and it must not silently drop the gate-2 not-attempted status just because it's a windowed companion endpoint", () => {
  const spec = agentToolSpec();
  const tool = spec.tools.find((t) => t.name === "voltrade_appstore_rankings_history");
  assert.ok(tool, "voltrade_appstore_rankings_history tool must exist");
  assert.deepEqual(tool.returns_provenance, ["data/appstore-rankings"], "must reuse the data/appstore-rankings license mark, not fork a duplicate one for the same watchlist under a different window");
  ...
  assert.equal(tool.input_schema.properties.days.maximum, 90, "must cap the lookback at the same 90-day bound the RAW /api/data/appstore-rankings/history route enforces");
  assert.ok(tool.input_schema.properties.ticker, "must expose the RAW route's per-app series lookup mode");
  const meta = apiMeta();
  const entry = meta.endpoints.find((e) => e.path === "/api/v1/data/appstore-rankings-history");
  assert.ok(entry, "voltrade_appstore_rankings_history must have a matching apiMeta().endpoints entry");
  assert.equal(entry.preview, "/api/data/appstore-rankings/history");
});
...
test("github-activity-history (accumulated GitHub org archive) shares the github-activity license mark and gate status — not a separate root, and it must not silently drop the gate-2 not-attempted status just because it's a windowed companion endpoint", () => {
  const spec = agentToolSpec();
  const tool = spec.tools.find((t) => t.name === "voltrade_github_activity_history");
  assert.ok(tool, "voltrade_github_activity_history tool must exist");
  assert.deepEqual(tool.returns_provenance, ["data/github-activity"], "must reuse the data/github-activity license mark, not fork a duplicate one for the same watchlist under a different window");
  assert.ok(tool.description.includes("GATE 1"), "honesty: gate-1 status must travel with the tool description");
  assert.ok(tool.description.includes("NOT-ATTEMPTED"), "honesty: gate-2's not-yet-attempted status must travel with this companion endpoint's description too");
  assert.equal(tool.input_schema.properties.weeks.maximum, 90, "must cap the lookback at the same 90-week bound the RAW /api/data/github-activity/history route enforces");
  assert.ok(tool.input_schema.properties.ticker, "must expose the RAW route's per-org series lookup mode");
  const meta = apiMeta();
  const entry = meta.endpoints.find((e) => e.path === "/api/v1/data/github-activity-history");
  assert.ok(entry, "voltrade_github_activity_history must have a matching apiMeta().endpoints entry");
  assert.equal(entry.preview, "/api/data/github-activity/history");
});
...
test("13f-holdings-history (SEC EDGAR 13F-HR accumulated archive) shares the 13f-holdings license mark and gate status — not a separate root, and it must not silently drop the gate-2 NOT-ATTEMPTED status or the full-holdings response-shape decision just because it's a windowed companion endpoint", () => {
  const spec = agentToolSpec();
  const tool = spec.tools.find((t) => t.name === "voltrade_thirteenf_holdings_history");
  assert.ok(tool, "voltrade_thirteenf_holdings_history tool must exist");
  assert.deepEqual(tool.returns_provenance, ["data/13f-holdings"], "must reuse the data/13f-holdings license mark, not fork a duplicate one for the same filings under a different window");
  assert.ok(tool.description.includes("GATE 1"), "honesty: gate-1-pass status must travel with the tool description");
  assert.ok(tool.description.includes("NOT been attempted"), "honesty: gate-2's not-yet-attempted status must travel with this companion endpoint's description too");
  assert.ok(tool.description.includes("FULL"), "honesty: the deliberate full-holdings (not top-25-trimmed) response shape must travel with this companion endpoint's description too — it must not silently regress to the RAW route's display trim");
  assert.equal(tool.input_schema.properties.days.maximum, 120, "must cap the lookback at the same 120-day bound the RAW /api/data/filings13f/history route enforces");
  const meta = apiMeta();
  const entry = meta.endpoints.find((e) => e.path === "/api/v1/data/13f-holdings-history");
  assert.ok(entry, "voltrade_thirteenf_holdings_history must have a matching apiMeta().endpoints entry");
  assert.equal(entry.preview, "/api/data/filings13f/history");
});
...
test("earnings-language-history (SEC 8-K accumulated archive) shares the earnings-language license mark and gate status — not a separate root, and it must not silently drop the gate-2 INCOMPLETE status just because it's a windowed companion endpoint", () => {
  const spec = agentToolSpec();
  const tool = spec.tools.find((t) => t.name === "voltrade_earnings_language_history");
  assert.ok(tool, "voltrade_earnings_language_history tool must exist");
  assert.deepEqual(tool.returns_provenance, ["data/earnings-language"], "must reuse the data/earnings-language license mark, not fork a duplicate one for the same filings under a different window");
  assert.ok(tool.description.includes("GATE 1"), "honesty: gate-1-pass status must travel with the tool description");
  assert.ok(tool.description.includes("INCOMPLETE"), "honesty: the gate-2 pilot's incomplete status must travel with this companion endpoint's description too");
  assert.equal(tool.input_schema.properties.days.maximum, 90, "must cap the lookback at the same 90-day bound the RAW /api/data/earnings-language/history route enforces");
  const meta = apiMeta();
...
test("attention-history (Wikimedia pageviews accumulated archive) shares the attention license mark and gate status — not a separate root, and it must not silently drop the gate-2 not-attempted status just because it's a windowed companion endpoint", () => {
  const spec = agentToolSpec();
  const tool = spec.tools.find((t) => t.name === "voltrade_attention_history");
  assert.ok(tool, "voltrade_attention_history tool must exist");
  assert.deepEqual(tool.returns_provenance, ["data/attention"], "must reuse the data/attention license mark, not fork a duplicate one for the same seed under a different window");
  assert.ok(tool.description.includes("GATE 1"), "honesty: gate-1-pass status must travel with the tool description");
  assert.ok(tool.description.includes("NOT-ATTEMPTED") || tool.description.includes("NOT ATTEMPTED"), "honesty: gate-2's not-yet-attempted status must travel with this companion endpoint's description too");
  assert.equal(tool.input_schema.properties.days.maximum, 90, "must cap the lookback at the same 90-day bound the RAW /api/data/attention/history route enforces");
  assert.ok(tool.input_schema.properties.ticker, "must expose the RAW route's per-ticker series lookup mode");
  const meta = apiMeta();
  const entry = meta.endpoints.find((e) => e.path === "/api/v1/data/attention-history");
  assert.ok(entry, "voltrade_attention_history must have a matching apiMeta().endpoints entry");
  assert.equal(entry.preview, "/api/data/attention/history");
});
...
test("GEM Global Coal Terminals Tracker license mark: CC BY 4.0 GEM data is freely resellable with attribution like methane-plumes/the government-produced streams, not conditional like the issuer-authored/informational-use-terms streams; agent tool documents the raw_only/no-ladder-gate state honestly", () => {
  assert.equal(LICENSE_MARKS["data/coal-terminals"].resell, "ok",
  ...
  assert.ok(LICENSE_MARKS["data/coal-terminals"].license.includes("CC BY 4.0"));
  assert.ok(LICENSE_MARKS["data/coal-terminals"].license.includes("Global Energy Monitor"));
  const spec = agentToolSpec();
  const tool = spec.tools.find((t) => t.name === "voltrade_coal_terminals");
  assert.ok(tool, "voltrade_coal_terminals tool must exist");
  assert.deepEqual(tool.returns_provenance, ["data/coal-terminals"]);
  ...
  assert.ok(tool.description.includes("NEVER inferring a role"), "honesty: the terminal-role classifier's drop-not-infer rule must travel with the tool description");
});

test("GEM Coal Mine Boundaries and Methane Sources license mark: CC BY 4.0 GEM data is freely resellable with attribution like coal-terminals/methane-plumes/the government-produced streams, not conditional like the issuer-authored/informational-use-terms streams; agent tool documents the raw_only/no-ladder-gate state honestly", () => {
  assert.equal(LICENSE_MARKS["data/coal-mine-features"].resell, "ok",
    "GEM publishes this release under CC BY 4.0 — must not be mismarked conditional like the issuer-authored or informational-use-terms streams");
  assert.ok(LICENSE_MARKS["data/coal-mine-features"].license.includes("CC BY 4.0"));
  assert.ok(LICENSE_MARKS["data/coal-mine-features"].license.includes("Global Energy Monitor"));
  const spec = agentToolSpec();
  const tool = spec.tools.find((t) => t.name === "voltrade_coal_mine_features");
  assert.ok(tool, "voltrade_coal_mine_features tool must exist");
  assert.deepEqual(tool.returns_provenance, ["data/coal-mine-features"]);
  assert.ok(tool.description.includes("no predictive claim"), "honesty: RAW catalogued geometry must not read as a trading signal");
  assert.ok(tool.description.includes("raw_only") || tool.description.includes("current_gate 0"), "honesty: the raw_only/no-ladder-gate state must travel with the tool description");
  assert.ok(tool.description.includes("dropped, never guessed"), "honesty: the id/geometry drop-not-guess rule must travel with the tool description");
});

test("GEM Global Iron Ore Mines Tracker license mark: CC BY 4.0 GEM data is freely resellable with attribution like coal-terminals/coal-mine-features/methane-plumes/the government-produced streams, not conditional like the issuer-authored/informational-use-terms streams; agent tool documents the raw_only/no-ladder-gate state honestly", () => {
  assert.equal(LICENSE_MARKS["data/iron-ore-mines"].resell, "ok",
    "GEM publishes this release under CC BY 4.0 — must not be mismarked conditional like the issuer-authored or informational-use-terms streams");
  assert.ok(LICENSE_MARKS["data/iron-ore-mines"].license.includes("CC BY 4.0"));
  assert.ok(LICENSE_MARKS["data/iron-ore-mines"].license.includes("Global Energy Monitor"));
  const spec = agentToolSpec();
  const tool = spec.tools.find((t) => t.name === "voltrade_iron_ore_mines");
  assert.ok(tool, "voltrade_iron_ore_mines tool must exist");
  assert.deepEqual(tool.returns_provenance, ["data/iron-ore-mines"]);
  assert.ok(tool.description.includes("no predictive claim"), "honesty: RAW catalogued reference data must not read as a trading signal");
  assert.ok(tool.description.includes("raw_only") || tool.description.includes("current_gate 0"), "honesty: the raw_only/no-ladder-gate state must travel with the tool description");
  assert.ok(tool.description.includes("never guessed"), "honesty: the operating-status classifier's degrade-to-unknown rule must travel with the tool description");
});

test("GEM Global Iron and Steel Tracker license mark: CC BY 4.0 GEM data is freely resellable with attribution like coal-terminals/coal-mine-features/iron-ore-mines/chemicals/methane-plumes/the government-produced streams, not conditional like the issuer-authored/informational-use-terms streams; agent tool documents the raw_only/no-ladder-gate state honestly", () => {
  assert.equal(LICENSE_MARKS["data/iron-steel-plants"].resell, "ok",
    "GEM publishes this release under CC BY 4.0 — must not be mismarked conditional like the issuer-authored or informational-use-terms streams");
  assert.ok(LICENSE_MARKS["data/iron-steel-plants"].license.includes("CC BY 4.0"));
  assert.ok(LICENSE_MARKS["data/iron-steel-plants"].license.includes("Global Energy Monitor"));
  const spec = agentToolSpec();
  const tool = spec.tools.find((t) => t.name === "voltrade_iron_steel_plants");
  assert.ok(tool, "voltrade_iron_steel_plants tool must exist");
  assert.deepEqual(tool.returns_provenance, ["data/iron-steel-plants"]);
  assert.ok(tool.description.includes("no predictive claim"), "honesty: RAW catalogued reference data must not read as a trading signal");
  assert.ok(tool.description.includes("raw_only") || tool.description.includes("current_gate 0"), "honesty: the raw_only/no-ladder-gate state must travel with the tool description");
...
test("GEM Production-Consumption of Met Coal & Iron Ore by Steel Industry license mark: CC BY 4.0 GEM data is freely resellable with attribution like coal-terminals/coal-mine-features/iron-ore-mines/chemicals/iron-steel-plants/lng-shipyards/methane-plumes/the government-produced streams, not conditional like the issuer-authored/informational-use-terms streams; agent tool documents the raw_only/no-ladder-gate state AND the country-choropleth (not point-layer) shape honestly", () => {
  assert.equal(LICENSE_MARKS["data/steel-raw-materials"].resell, "ok",
    "GEM publishes this release under CC BY 4.0 — must not be mismarked conditional like the issuer-authored or informational-use-terms streams");
  assert.ok(LICENSE_MARKS["data/steel-raw-materials"].license.includes("CC BY 4.0"));
  assert.ok(LICENSE_MARKS["data/steel-raw-materials"].license.includes("Global Energy Monitor"));
  const spec = agentToolSpec();
  const tool = spec.tools.find((t) => t.name === "voltrade_steel_raw_materials");
  assert.ok(tool, "voltrade_steel_raw_materials tool must exist");
  assert.deepEqual(tool.returns_provenance, ["data/steel-raw-materials"]);
  assert.ok(tool.description.includes("no predictive claim"), "honesty: RAW catalogued reference data must not read as a trading signal");
  assert.ok(tool.description.includes("raw_only") || tool.description.includes("current_gate 0"), "honesty: the raw_only/no-ladder-gate state must travel with the tool description");
  assert.ok(tool.description.includes("no per-facility coordinates") || tool.description.toLowerCase().includes("choropleth"), "honesty: the country-choropleth (not point-layer) shape must travel with the tool description, not just live in code comments");
});
...
test("openapi spec: full coverage — every one of the 41 live tools now has a hand-verified RESPONSE_DATA_SCHEMAS entry, no stale extras", () => {
